/*
 * Visual Adding Game - One Split Version
 * 
 * Modified behavior:
 * - Players must perform at least ONE split (any split, not just tens/ones)
 * - After the first split, players can combine numbers freely
 * - Splitting can break any number > 1 into any two smaller numbers that add up to it
 * - No strict combination rules after the first split occurs
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let problemCount = 0;
    let hasSplitOnce = false; // Track if at least one split has occurred
    ...
    function generateProblem() {
        ...
        currentNumbers = [a, b];
        hasSplitOnce = false; // Reset split state for new problem
        ...
        document.querySelectorAll('.number-circle').forEach(circle => {
            ...
            const handleTap = function(event) {
                ...
                if (tapCount === 1) {
                    ...
                    if (idx !== -1 && num > 1) { // Allow splitting any number > 1
                        showSplitOptions(num, idx);
                    }
                }
            };
    ...
    function isAllowedToAdd(a, b, len) {
        // If no split has occurred yet, don't allow any combinations
        if (!hasSplitOnce) {
            return false;
        }
        
        // After at least one split, allow any combination
        return true;
    }
    
    // Function to show split options for a number
    function showSplitOptions(num, idx) {
        // Priority 1: Tens/ones split if it makes sense (number >= 10 and has ones)
        ...
        if (tens > 0 && ones > 0) {
            ...
            // Priority 2: Split into two roughly equal parts
            const half = Math.floor(num / 2);
            const remainder = num - half;
            currentNumbers.splice(idx, 1, half, remainder);
        }
        
        hasSplitOnce = true; // Mark that a split has occurred
        renderEquation();
        makeDraggable();
    }

document.addEventListener('DOMContentLoaded', () => {
    ...
    let problemCount = 0;
    let hasSplitInMethod2 = false; // Track if a number has been split in Method 2
    
    // Question format configuration - cycle through these combinations
    // Each array element is [show_blocks, method_type, generator_function_name]
    // method_type: 0 = no splitting, 1 = Method 1 (split both), 2 = Method 2 (split one)
    let question_format = [
        [1, 1, "generateDoublePlusDoubleWithCarry"],
        [1, 2, "generateDoublePlusDoubleWithCarry"],
        ];
    ...
    function getCurrentSettings() {
        ...
        const [show_blocks, method_type, generator_name] = question_format[index];
        const generator = generatorFunctions[generator_name];
        return { show_blocks, method_type, generator };
    }
    ...
    function generateProblem() {
        let a, b;
        const { generator, method_type } = getCurrentSettings();
        [a, b] = generator(); // Use the selected generator function
        currentNumbers = [a, b];
        hasSplitInMethod2 = false; // Reset Method 2 tracking
        ...
        if (method_type === 1) {
            methodIndicator.textContent = 'Method 1';
            methodIndicator.classList.remove('hidden');
        } else if (method_type === 2) {
            methodIndicator.textContent = 'Method 2';
            methodIndicator.classList.remove('hidden');
        } else {
            methodIndicator.classList.add('hidden');
        }
    ...
    function renderEquation() {
        ...
        const { show_blocks, method_type } = getCurrentSettings();
        ...
        document.querySelectorAll('.number-circle').forEach(circle => {
            ...
            const handleTap = function(event) {
                ...
                if (tapCount === 1) {
                    ...
                    if (idx !== -1) {
                        const { method_type } = getCurrentSettings();
                        
                        // Check Method 2 constraint: only allow splitting if no number has been split yet
                        if (method_type === 2 && hasSplitInMethod2) {
                            return; // Don't allow splitting if already split in Method 2
                        }
                        ...
                        if (tens > 0 && ones > 0) {
                            ...
                            // Mark that we've split in Method 2
                            if (method_type === 2) {
                                hasSplitInMethod2 = true;
                            }
    ...
    function isAllowedToAdd(a, b, len) {
        ...
        // Method 2 rules: Allow combining split parts with whole numbers
        if (method_type === 2) {
            // Allow tens with tens, ones with ones (same as Method 1)
            if (isMult10(a) && isMult10(b)) return true;
            if (isSingle(a) && isSingle(b)) return true;
            
            // Allow combining a split part (tens or ones) with any double-digit number
            if ((isMult10(a) || isSingle(a)) && isDoubleDigit(b)) return true;
            if ((isMult10(b) || isSingle(b)) && isDoubleDigit(a)) return true;
            
            return false;
        }
        
        // Method 0 (no splitting) - no drag and drop allowed
        return false;
    }

document.addEventListener('DOMContentLoaded', () => {
    ...
    // === GAME VARIABLES ===
    const skip_counting = true; // Set to true to use number board, false to use numpad
    let currentNumbers = [];
    let droppedFlag = false;
    let problemCount = 0;
    let expectedAnswer = 0; // For number board mode
    ...
    function showPopup(a, b) {
        ...
        if (skip_counting) {
            // Show number board mode
            document.getElementById('numpad').classList.add('hidden');
            document.getElementById('number-board').classList.remove('hidden');
            document.getElementById('answer-input').style.display = 'none';
            popup.classList.add('number-board-mode');
            initializeNumberBoard(a);
        } else {
            // Show numpad mode
            document.getElementById('numpad').classList.remove('hidden');
            document.getElementById('number-board').classList.add('hidden');
            document.getElementById('answer-input').style.display = 'block';
            popup.classList.remove('number-board-mode');
            answerInput.value = '';
            setupNumpadHandlers(a, b);
        }
    ...
    function setupNumpadHandlers(a, b) {
        ...
        document.getElementById('submit').onclick = () => {
            ...
            if (isNaN(ans) || ans !== a + b) {
                ...
                handleCorrectAnswer(a, b);
            }
        };
    }
    
    function initializeNumberBoard(startingNumber = 0) {
        ...
        for (let i = 1; i <= 100; i++) {
            ...
            // Pre-color the starting number tiles (1 to startingNumber)
            if (i <= startingNumber) {
                tile.classList.add('starting-position');
            }
            
            tile.addEventListener('click', () => {
                handleNumberBoardClick(tile, i);
            });
    ...
    function handleNumberBoardClick(tile, number) {
        if (number === expectedAnswer) {
            // Correct answer
            tile.classList.add('correct');
            setTimeout(() => {
                const a = Math.min(...currentNumbers.slice(-2));
                const b = Math.max(...currentNumbers.slice(-2));
                handleCorrectAnswer(a, b);
            }, 500);
        } else {
            // Incorrect answer
            showIncorrectFeedback(tile);
        }
    }
    
    function showIncorrectFeedback(tile) {
